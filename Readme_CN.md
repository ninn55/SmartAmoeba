...
本文档还包含 tinyML 的一般介绍和概述。

名称解释here。

介绍
将 AI（人工智能）迁移到嵌入式应用程序可能会带来某些挑战。与云计算和移动计算相比，嵌入式硬件的生态系统可能是碎片化的。系统可以从软件到硬件堆栈，从构建系统到编译器，从 ISA 到外围设备。嵌入式设备的核心，是一种针对广泛应用的高度定制化的计算机系统，从而导致了高度定制化的软硬件设计。OS（操作系统）从裸机到 RTOS（实时操作系统），再到微内核，再到 Linux，旨在适应不同的硬件。功耗可以从微瓦到数百瓦不等，以适应从感官分析到自动驾驶汽车和无人机的需求。典型的嵌入式系统具有以下特征：

有限的计算资源：AI/ML 算法通常需要更多的计算。运行程序需要 CPU（中央计算单元）时间和 RAM（随机访问内存）。就其而言，它们由有限的权力决定。在将 AI 算法移植到边缘时，这将带来重大挑战。如下表所示，通过Cloud Mobile和Tiny对比，可以很明显的看出计算能力的差异。
...
缺乏可移植性：用于商业用途的软件堆栈始终面向业务逻辑。为了速度和空间而牺牲了便携性。
